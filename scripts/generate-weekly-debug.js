...
function normalizeWeekly(w){const customTypes=Array.from(new Set((w.customTypes||[]).map(s=>String(s).trim()).filter(Boolean)));const benchmarks=Object.assign({},w.benchmarks||{});customTypes.forEach(t=>{if(!(t in benchmarks))benchmarks[t]=0});const days=(w.days||[]).map(d=>{const types={};Object.keys(d.types||{}).forEach(k=>{const kk=String(k).trim();if(!kk)return;types[kk]=!!d.types[k]});const rawList=Array.isArray(d.sessionsList)?d.sessionsList:[];const seen=new Set();const sessionsList=rawList.reduce((acc,s)=>{const key=s?.id?s.id:JSON.stringify(s?.sessionTypes||s);if(seen.has(key))return acc;seen.add(key);acc.push({id:s?.id,sessionTypes:Array.isArray(s?.sessionTypes)?s.sessionTypes:[]});return acc;},[]);return {...d,types,sessions:typeof d.sessions==='number'?d.sessions:(sessionsList.length||0),sessionsList}});return {...w,customTypes,benchmarks,days};}
